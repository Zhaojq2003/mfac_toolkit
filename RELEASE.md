...
构建出的 wheel 包含 Python 源码与预编译扩展，产物位于 `dist/`。

## GitHub Release 发布步骤

1. 确认 `uv run ruff check .` 与 `uv run mypy .` 通过。
2. 创建并推送 tag：
...
3. 在 GitHub Releases 页面基于该 tag 创建 Release。
4. 上传 `dist/` 中的 wheel 文件作为 release asset。
5. 用户通过以下命令安装：
