...
"""MIMO MFAC 解耦示例：2×2 线性耦合系统闭环仿真.

被控对象存在交叉耦合，MIMO-CFDL 通过估计 PJM 矩阵实现解耦控制。

运行: uv run python -m mfac_toolkit.examples.mimo_coupled_example
"""
...
def plot_result(
    ...
    """保存两个通道的跟踪曲线、控制输入与跟踪误差图."""
    dim = y.shape[1]
    fig, axes = plt.subplots(3, dim, figsize=(10, 8), sharex=True)
    ...
    for i in range(dim):
        ...
        axes[2, i].plot(t, yd[:, i] - y[:, i])
        axes[2, i].axhline(0.0, color="k", linestyle="--", linewidth=0.8)
        axes[2, i].set_xlabel("Time [s]")
        axes[2, i].set_ylabel(f"e[{i}]")
...
def main() -> None:
    ...
    # 2×2 线性耦合被控对象：y = G @ u
    plant = MimoLinearPlant(gain=np.array([[0.5, 0.3], [0.2, 0.6]], dtype=np.float64))
    ...
    out_path = Path(__file__).parent / "output" / "mimo_coupled_example.png"
